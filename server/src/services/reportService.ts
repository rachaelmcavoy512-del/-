...
function d2n(d: Prisma.Decimal | null | undefined): number {
  if (!d) return 0;
...
function rangeOf(year: number, period: number, rangeType: string): { start: Date; end: Date; label: string } {
...
async function aggregateByAccount(
...
): Promise<Map<number, { debit: Prisma.Decimal; credit: Prisma.Decimal }>> {
  const rows = await prisma.voucherEntry.groupBy({
    by: ['accountId'],
    where: {
      voucher: {
        subjectId,
  ...
  const map = new Map<number, { debit: Prisma.Decimal; credit: Prisma.Decimal }>();
  for (const r of rows) {
    map.set(r.accountId, {
      debit: r._sum.debit ?? new Prisma.Decimal(0),
      credit: r._sum.credit ?? new Prisma.Decimal(0),
    });
  }
  return map;
}
...
export interface BalanceSheetItem {
  ...
  balance: number;
  lineNo: number;
  computed?: boolean; // 是否为计算行（如未分配利润）
}
...
export interface IncomeStatementResult {
  ...
  periodExpenses: number; // 期间费用（销售+管理+财务）
...
export interface CashFlowPart {
  ...
  outflow: number;
  net: number;
  items: Array<{ code: string; name: string; inflow: number; outflow: number }>;
}
...
export async function computeBalance(
  ...
  if (upToEndDate) dateFilter.lte = new Date(upToEndDate);
  if (fromStartDate) dateFilter.gte = new Date(fromStartDate);
...
export async function getBalanceSheet(
  ...
  for (const acc of accounts) {
    ...
    if (acc.category === 'INCOME') {
      ...
      if (a) incomeSum = incomeSum.add(signedBalance(acc.balanceDirection, a.debit, a.credit));
    } else if (acc.category === 'EXPENSE') {
      const a = ytd.get(acc.id);
      if (a) expenseSum = expenseSum.add(signedBalance(acc.balanceDirection, a.debit, a.credit));
    }
  }
  ...
  const equities: BalanceSheetItem[] = [];
  ...
  for (const acc of accounts) {
    ...
    const debit = a?.debit ?? new Prisma.Decimal(0);
    const credit = a?.credit ?? new Prisma.Decimal(0);
    // 按科目类别自然方向计算余额：资产/成本类借方为正，负债/权益类贷方为正
    // 备抵科目（如累计折旧贷方余额、进项税额借方余额）会以负数正确抵减所属类别
    const bal =
      acc.category === 'ASSET' || acc.category === 'COST'
        ? debit.sub(credit)
        : credit.sub(debit);
...
export async function getIncomeStatement(
  ...
  const incomes: IncomeStatementItem[] = [];
  const expenses: IncomeStatementItem[] = [];
  let operatingRevenue = new Prisma.Decimal(0); // 主营+其他业务收入
  let operatingCost = new Prisma.Decimal(0); // 主营+其他业务成本
  let taxAndSurcharge = new Prisma.Decimal(0); // 税金及附加
  let sellingExpense = new Prisma.Decimal(0); // 销售费用
  let adminExpense = new Prisma.Decimal(0); // 管理费用
  let finExpense = new Prisma.Decimal(0); // 财务费用
  let investIncome = new Prisma.Decimal(0); // 投资收益
  let nonOpIncome = new Prisma.Decimal(0); // 营业外收入
  let nonOpExpense = new Prisma.Decimal(0); // 营业外支出
  let incomeTax = new Prisma.Decimal(0); // 所得税费用
  ...
  for (const acc of accounts) {
    ...
    if (acc.category === 'INCOME') {
      ...
      if (acc.code === '6001' || acc.code === '6051') operatingRevenue = operatingRevenue.add(amount);
      if (acc.code === '6111') investIncome = investIncome.add(amount);
      if (acc.code === '6301') nonOpIncome = nonOpIncome.add(amount);
    } else {
      expenses.push({ code: acc.code, name: acc.name, amount: amtN });
      if (acc.code === '6401' || acc.code === '6402') operatingCost = operatingCost.add(amount);
      if (acc.code === '6403') taxAndSurcharge = taxAndSurcharge.add(amount);
      if (acc.code === '6601') sellingExpense = sellingExpense.add(amount);
      if (acc.code === '6602') adminExpense = adminExpense.add(amount);
      if (acc.code === '6603') finExpense = finExpense.add(amount);
      if (acc.code === '6711') nonOpExpense = nonOpExpense.add(amount);
      if (acc.code === '6801') incomeTax = incomeTax.add(amount);
    }
  }
...
export async function getCashFlowStatement(
  ...
  const cashAccountIds = new Set(
    accounts.filter((a) => CASH_CODES.includes(a.code)).map((a) => a.id)
  );
  ...
  const operating: CashFlowPart = { inflow: 0, outflow: 0, net: 0, items: [] };
  const investing: CashFlowPart = { inflow: 0, outflow: 0, net: 0, items: [] };
  const financing: CashFlowPart = { inflow: 0, outflow: 0, net: 0, items: [] };

  // 活动类型 -> part 映射
  const partMap: Record<string, CashFlowPart> = {
    operating,
    investing,
    financing,
  };
  ...
  const itemMap = new Map<
    string,
    { code: string; name: string; inflow: number; outflow: number; partKey: string }
  >();
    ...
    for (const ce of cashEntries) {
      ...
      if (otherEntries.length > 0) {
        ...
        const sorted = [...otherEntries].sort(
          (a, b) => Number(b.debit.add(b.credit).toString()) - Number(a.debit.add(a.credit).toString())
        );
        const top = sorted[0];
...
function classifyActivity(code: string, _category: string): string {
...
export async function getTrialBalance(
  ...
  for (const acc of accounts) {
    ...
    const oDebit = o?.debit ?? new Prisma.Decimal(0);
    const oCredit = o?.credit ?? new Prisma.Decimal(0);
    const pDebit = p?.debit ?? new Prisma.Decimal(0);
    const pCredit = p?.credit ?? new Prisma.Decimal(0);

    // 期初带符号余额
    const openingBal = signedBalance(acc.balanceDirection, oDebit, oCredit);
    // 期末 = 期初 + 本期发生（按方向）
    const closingBal =
      acc.balanceDirection === 'DEBIT'
        ? openingBal.add(pDebit).sub(pCredit)
        : openingBal.add(pCredit).sub(pDebit);

    // 按余额方向拆借/贷列
    const openingDebit = openingBal.gte(0) && acc.balanceDirection === 'DEBIT' ? openingBal : new Prisma.Decimal(0);
    const openingCredit = openingBal.gte(0) && acc.balanceDirection === 'CREDIT' ? openingBal : new Prisma.Decimal(0);
    const closingDebit = closingBal.gte(0) && acc.balanceDirection === 'DEBIT' ? closingBal : new Prisma.Decimal(0);
    const closingCredit = closingBal.gte(0) && acc.balanceDirection === 'CREDIT' ? closingBal : new Prisma.Decimal(0);
...
export async function getGeneralLedger(
  ...
  if (rangeType === 'QUARTER') {
    ...
    months = [];
    for (let m = 1; m <= period; m++) months.push(m);
  } else {
    months = [period];
  }
  ...
  const openingBalance = signedBalance(
    ...
    o?.debit ?? new Prisma.Decimal(0),
    o?.credit ?? new Prisma.Decimal(0)
  );
  ...
  for (const m of months) {
    ...
    const d = a?.debit ?? new Prisma.Decimal(0);
    const c = a?.credit ?? new Prisma.Decimal(0);
    // 按方向累加
    runningBalance =
...
export async function getSubsidiaryLedger(
  ...
  const resultEntries: SubsidiaryLedgerEntry[] = [];
