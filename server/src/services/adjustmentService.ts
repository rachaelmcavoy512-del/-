...
import { createVoucher, postVoucher } from './voucherService';

// 计入余额的凭证状态
const POSTED_STATUSES = ['POSTED', 'RED_VOID'];

// 获取进项税额科目余额（借方余额）
async function getInputTaxBalance(subjectId: number): Promise<{ accountId: number; balance: number }> {
...
export async function generateAdjustmentVoucher(eventId: number, userId: number) {
  ...
  // 获取主营业务成本科目
  const costAccount = await prisma.account.findFirst({
  ...
  // 创建调整凭证（草稿）
  const voucher = await createVoucher(event.subjectId, {
...
// 过账调整凭证并将事件标记为已整改
export async function postAdjustmentAndResolve(eventId: number, userId: number) {
  ...
  // 过账调整凭证
  const voucher = await postVoucher(event.adjustmentVoucherId, userId);
  // 事件状态置为已整改
  await prisma.riskEvent.update({
    where: { id: eventId },
    data: {
