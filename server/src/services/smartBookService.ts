...
import { createVoucher, postVoucher, serializeVoucher } from './voucherService';
...
export interface PreviewBooking {
  ...
  sourceId: number;
  sourceDesc: string; // 源数据描述
  voucherDate: string;
  voucherSummary: string;
  entries: PreviewEntry[];
  identifiable: boolean; // false 表示无法识别（银行流水）
}
...
// 按主体+编码查找科目
async function findAccount(subjectId: number, code: string) {
  return prisma.account.findFirst({ where: { subjectId, code } });
}

// 判断是否一般纳税人
async function isGeneralTaxpayer(subjectId: number): Promise<boolean> {
...
export async function previewBookings(
...
): Promise<{ bookings: PreviewBooking[]; unidentified: UnidentifiedBankTxn[] }> {
  const general = await isGeneralTaxpayer(subjectId);
  const bookings: PreviewBooking[] = [];
  const unidentified: UnidentifiedBankTxn[] = [];

  // 预加载常用科目
  const accountCache = new Map<string, { id: number; code: string; name: string }>();
  async function getAccount(code: string) {
    if (!accountCache.has(code)) {
      const acc = await findAccount(subjectId, code);
      if (acc) accountCache.set(code, { id: acc.id, code: acc.code, name: acc.name });
    }
  ...
  if (!dataType || dataType === 'INVOICE') {
    ...
    for (const inv of invoices) {
      ...
      if (inv.direction === 'OUTPUT') {
        ...
        if (!ar || !rev || !taxAcc) continue; // 科目缺失跳过
        bookings.push({
          sourceType: 'INVOICE',
          sourceId: inv.id,
  ...
  if (!dataType || dataType === 'BANK') {
    ...
    // 预加载购销方名称用于匹配
    const outputBuyers = await prisma.invoice.findMany({
      where: { subjectId, direction: 'OUTPUT' },
      select: { buyerName: true },
      distinct: ['buyerName'],
    });
    const inputSellers = await prisma.invoice.findMany({
      where: { subjectId, direction: 'INPUT' },
      select: { sellerName: true },
      distinct: ['sellerName'],
    });
    const buyerSet = new Set(outputBuyers.map((i) => i.buyerName).filter(Boolean) as string[]);
    const sellerSet = new Set(inputSellers.map((i) => i.sellerName).filter(Boolean) as string[]);
  ...
  if (!dataType || dataType === 'PAYROLL') {
    ...
    // 按 period 分组
    const periodMap = new Map<string, { gross: number; tax: number; net: number; count: number }>();
...
export async function generateBookings(
  ...
  userId: number,
  dataType?: 'INVOICE' | 'BANK' | 'PAYROLL'
): Promise<{ generated: number; vouchers: unknown[] }> {
  ...
  for (const bk of invoiceBookings) {
    ...
    try {
      const voucher = await createVoucher(subjectId, {
...
export async function getPendingReview(subjectId: number) {
...
// 批量过账草稿凭证
export async function batchPost(voucherIds: number[], userId: number) {
  const results: Array<{ id: number; success: boolean; error?: string }> = [];
  for (const id of voucherIds) {
    try {
      await postVoucher(id, userId);
...
export async function getUnidentified(subjectId: number): Promise<UnidentifiedBankTxn[]> {
...
export async function assignManualEntry(
  subjectId: number,
  ...
  let entries: Array<{ accountId: number; summary: string; debit: number; credit: number }>;
  if (txn.direction === 'IN') {
    entries = [
