...
import { onVoucherPosted } from './riskScanService';
...
export interface CreateVoucherInput {
  ...
  summary?: string | null;
  attachments?: number;
  entries: CreateVoucherEntryInput[];
}
...
async function aggregatePosted(
  subjectId: number,
  accountId: number,
...
export async function createVoucher(
  ...
  for (const e of data.entries) {
    sumDebit = sumDebit.add(new Prisma.Decimal(e.debit || 0));
    sumCredit = sumCredit.add(new Prisma.Decimal(e.credit || 0));
  }
  // 借贷必须平衡且 > 0
...
// 余额由实时聚合计算，过账仅变更状态
export async function postVoucher(voucherId: number, userId: number) {
  const voucher = await prisma.voucher.findUnique({ where: { id: voucherId } });
  if (!voucher) {
    throw new Error('凭证不存在');
  }
  ...
  onVoucherPosted(updated.subjectId);
  return serializeVoucher(updated);
}

// 作废凭证：仅 DRAFT 可作废（直接置 VOID）
// 已过账凭证不能直接作废，需走红冲
export async function voidVoucher(voucherId: number, _userId: number) {
  const voucher = await prisma.voucher.findUnique({ where: { id: voucherId } });
  if (!voucher) {
    throw new Error('凭证不存在');
  }
...
export async function redOffsetVoucher(
  voucherId: number,
  userId: number,
  reason?: string
) {
  ...
  const redVoucher = await prisma.voucher.create({
    data: {
...
export async function periodClose(
  subjectId: number,
  year: number,
  month: number,
  userId: number
  ...
  // 获取本年利润科目
  const profitAccount = await prisma.account.findFirst({
  ...
  for (const acc of pnlAccounts) {
    ...
    let balance: Prisma.Decimal; // 按自然方向的带符号余额
    ...
    if (absBal.lte(0)) continue; // 无余额不结转

    const isIncome = acc.category === 'INCOME';
    let accountDebit = new Prisma.Decimal(0);
    let accountCredit = new Prisma.Decimal(0);
...
export async function getSubjectBalances(subjectId: number, year: number, month: number) {
  const monthStart = new Date(year, month - 1, 1, 0, 0, 0, 0);
  const monthEnd = new Date(year, month, 0, 23, 59, 59, 999);
