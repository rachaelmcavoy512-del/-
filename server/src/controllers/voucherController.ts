// 凭证记账控制器（Task3）
// 处理 HTTP 请求/响应，zod 校验 body，调用 voucherService 完成业务逻辑
...
// 凭证状态取值
const STATUSES = ['DRAFT', 'POSTED', 'VOID', 'RED_VOID'] as const;
...
const createSchema = z.object({
  ...
  entries: z.array(entrySchema).min(1, '至少一条分录'),
});
...
// 红冲校验
const redOffsetSchema = z.object({
...
export async function create(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const voucher = await createVoucher(
      subjectId,
      { voucherDate, summary: summary ?? null, attachments, entries },
      userId
    );
...
export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (req.query.from || req.query.to) {
      ...
      if (req.query.from) voucherDate.gte = new Date(String(req.query.from));
      if (req.query.to) voucherDate.lte = new Date(String(req.query.to));
...
export async function detail(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const amount = voucher.entries.reduce(
      (sum, e) => sum.plus(e.debit),
      new Prisma.Decimal(0)
    );
...
export async function post(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const userId = req.user!.id;
    const voucher = await postVoucher(id, userId);
...
export async function voidVoucherCtrl(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
      return res.status(400).json({ error: '无效的凭证 ID' });
    ...
    const userId = req.user!.id;
    const voucher = await voidVoucher(id, userId);
    return res.json(voucher);
  } catch (err) {
    return handleServiceError(res, err) || next(err);
  }
...
export async function redOffset(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
      return res.status(400).json({ error: '无效的凭证 ID' });
    ...
    const userId = req.user!.id;
    const voucher = await redOffsetVoucher(id, userId, parsed.data.reason);
    return res.status(201).json(voucher);
  } catch (err) {
    return handleServiceError(res, err) || next(err);
  }
...
export async function periodCloseCtrl(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const userId = req.user!.id;
    const voucher = await periodClose(subjectId, year, month, userId);
    return res.status(201).json(voucher);
  } catch (err) {
    return handleServiceError(res, err) || next(err);
  }
...
export async function balances(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const result = await getSubjectBalances(subjectId, year, month);
...
export async function accountBalance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const balance = await getAccountBalance(subjectId, accountId, upToDate);
