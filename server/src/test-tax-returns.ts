...
import { importInvoices } from './services/importService';
...
async function main() {
  ...
  await prisma.taxReturn.deleteMany({ where: { subject: { taxNumber } } }).catch(() => undefined);
  ...
  await prisma.taxReturn.deleteMany({ where: { subjectId } });
  console.log('已清理历史发票/凭证/工资/申报数据');

  // 1. 导入发票（销项 10000+1300，进项 5000+650）
  const invResult = await importInvoices(subjectId, buildInvoiceXml(), 'tax_test_invoices.xml', admin.id);
  ...
  try {
    ...
    console.log(
      `  销项税额=${vatSummary.outputTax}, 进项税额=${vatSummary.inputTax}, 应纳税额=${vatSummary.taxAmount}, 期末留抵=${vatSummary.endingCredit}`
    );
    ...
    console.log(
      `  计税依据(增值税)=${surReport.vatPayable}, 城建税=${surReport.items[0].amount}, 教育费附加=${surReport.items[1].amount}, 地方教育附加=${surReport.items[2].amount}, 合计=${surReport.total}`
    );
    ...
    console.log(
      `  营业收入=${citReport.revenue}, 营业成本=${citReport.cost}, 期间费用=${citReport.expenses}, 利润=${citReport.profit}, 应纳所得税=${citReport.taxAmount}`
    );
    ...
    const listResp = await request(
      'GET',
      `/api/tax-returns?subjectId=${subjectId}&pageSize=50`,
      token
    );
    assert(listResp.status === 200, '列表查询返回 200');
    assert(listResp.data.total === 3, `列表共3条申报记录（实际 ${listResp.data.total}）`);
    const vatListItem = listResp.data.items.find((i: any) => i.taxType === 'VAT' && i.period === '2026-06');
    ...
    console.log('\n--- 测试8：应纳增值税额预览接口 ---');
    const vpResp = await request(
      'GET',
      `/api/tax-returns/vat-payable?subjectId=${subjectId}&period=2026-06`,
      token
    );
