...
async function main() {
    ...
    // 2. 准备测试主体（复用或新建）
    let subject = await prisma_1.default.taxpayerSubject.findFirst({
    ...
    await prisma_1.default.riskRemediation.deleteMany({ where: { event: { subjectId: sid } } });
    await prisma_1.default.riskEvent.deleteMany({ where: { subjectId: sid } });
    await prisma_1.default.invoice.deleteMany({ where: { subjectId: sid } });
    await prisma_1.default.bankTransaction.deleteMany({ where: { subjectId: sid } });
    ...
    const summary = await (0, riskScanService_1.scanSubject)(sid, undefined, CUR);
    console.log(`  扫描 ${summary.scanned} 个指标，命中 ${summary.hit} 个风险`);
    console.log('  命中事件：');
    summary.events.forEach((e) => console.log(`    - event#${e.id} ${e.indicatorCode} | level=${e.level} | status=${e.status}`));
    // 6. 校验事件等级
    console.log('\n[5] 校验风险事件等级...');
    ...
    const alerts = await prisma_1.default.auditLog.findMany({
    ...
    if (!highEvent) {
        ...
        const rem = await prisma_1.default.riskRemediation.create({
        ...
        let ev = await prisma_1.default.riskEvent.findUnique({ where: { id: highEvent.id } });
        console.log(`  事件状态：${ev?.status}（期望 IN_PROGRESS）`);
        // 标记整改完成
        await prisma_1.default.riskRemediation.update({ where: { id: rem.id }, data: { status: 'DONE' } });
        console.log(`  整改任务标记 DONE → 事件应自动 RESOLVED`);
        ev = await prisma_1.default.riskEvent.findUnique({ where: { id: highEvent.id } });
    ...
    // 9. 看板统计
    console.log('\n[8] 风险看板统计...');
    const allEvents = await prisma_1.default.riskEvent.findMany({ where: { subjectId: sid } });
