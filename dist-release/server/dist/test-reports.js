...
async function main() {
    ...
    try {
        ...
        const bsResp = await request('GET', `/api/reports/balance-sheet?subjectId=${subjectId}&year=2026&period=6`, token);
        assert(bsResp.status === 200, `资产负债表接口返回 200（实际 ${bsResp.status}）`);
        const bs = bsResp.data;
        console.log(`  资产合计=${bs.totalAssets}, 负债合计=${bs.totalLiabilities}, 所有者权益合计=${bs.totalEquity}, 负债+权益=${bs.totalLiabilities + bs.totalEquity}`);
        ...
        // ===== 测试2：利润表（MONTH）=====
        console.log('\n--- 测试2：利润表（2026-06 单月）---');
        const isResp = await request('GET', `/api/reports/income-statement?subjectId=${subjectId}&year=2026&period=6&rangeType=MONTH`, token);
        assert(isResp.status === 200, `利润表接口返回 200（实际 ${isResp.status}）`);
        const is = isResp.data;
        console.log(`  营业收入=${is.operatingRevenue}, 营业成本=${is.operatingCost}, 期间费用=${is.periodExpenses}, 营业利润=${is.operatingProfit}, 利润总额=${is.totalProfit}, 净利润=${is.netProfit}`);
        ...
        const cfResp = await request('GET', `/api/reports/cash-flow-statement?subjectId=${subjectId}&year=2026&period=6&rangeType=MONTH`, token);
        assert(cfResp.status === 200, `现金流量表接口返回 200（实际 ${cfResp.status}）`);
        const cf = cfResp.data;
        console.log(`  经营：流入=${cf.operating.inflow} 流出=${cf.operating.outflow} 净额=${cf.operating.net}; 投资：净额=${cf.investing.net}; 筹资：流入=${cf.financing.inflow} 流出=${cf.financing.outflow} 净额=${cf.financing.net}; 现金净增加=${cf.netChange}`);
        ...
        const tbResp = await request('GET', `/api/reports/trial-balance?subjectId=${subjectId}&year=2026&period=6`, token);
        assert(tbResp.status === 200, `试算平衡表接口返回 200（实际 ${tbResp.status}）`);
        const tb = tbResp.data;
        console.log(`  本期借方合计=${tb.totalPeriodDebit}, 本期贷方合计=${tb.totalPeriodCredit}`);
        ...
        const glResp = await request('GET', `/api/reports/general-ledger?subjectId=${subjectId}&accountId=${acct.cash}&year=2026&period=6&rangeType=MONTH`, token);
        assert(glResp.status === 200, `总账接口返回 200（实际 ${glResp.status}）`);
        const gl = glResp.data;
        console.log(`  科目=${gl.account.code} ${gl.account.name}, 期初=${gl.openingBalance}, 借方合计=${gl.totalDebit}, 贷方合计=${gl.totalCredit}, 期末=${gl.closingBalance}`);
        ...
        const slResp = await request('GET', `/api/reports/subsidiary-ledger?subjectId=${subjectId}&accountId=${acct.cash}&from=2026-06-01&to=2026-06-30`, token);
        assert(slResp.status === 200, `明细账接口返回 200（实际 ${slResp.status}）`);
        const sl = slResp.data;
        console.log(`  科目=${sl.account.code} ${sl.account.name}, 期初=${sl.openingBalance}, 凭证笔数=${sl.entries.length}, 借方合计=${sl.totalDebit}, 贷方合计=${sl.totalCredit}, 期末=${sl.closingBalance}`);
        ...
        const isYearResp = await request('GET', `/api/reports/income-statement?subjectId=${subjectId}&year=2026&period=6&rangeType=YEAR`, token);
