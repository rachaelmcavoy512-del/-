...
exports.scanSubject = scanSubject;
exports.rescanAll = rescanAll;
exports.triggerScan = triggerScan;
exports.onVoucherPosted = onVoucherPosted;
exports.onDataImported = onDataImported;
...
function matchNumericThreshold(threshold, value) {
    if (threshold.lt !== undefined)
        return value < Number(threshold.lt);
    if (threshold.gt !== undefined)
        return value > Number(threshold.gt);
    // 兼容 creditRatioGt / costRatioGt / diffGt 等领域键名
    for (const key of Object.keys(threshold)) {
        if (key.endsWith('Gt') && threshold[key] !== undefined)
            return value > Number(threshold[key]);
        if (key.endsWith('Lt') && threshold[key] !== undefined)
            return value < Number(threshold[key]);
    }
    return false;
}
// 解析所属期（YYYY-MM）。若未提供，则取该主体最新发票/凭证所在月份，兜底当前月
async function resolvePeriod(subjectId, period) {
    if (period && /^\d{4}-\d{2}$/.test(period))
        return period;
    // 取最新发票开票月份
    const latestInvoice = await prisma_1.default.invoice.findFirst({
        where: { subjectId },
...
async function aggregateInvoices(subjectId, start, end) {
...
async function aggregatePnl(subjectId, start, end) {
...
async function detectVatTaxBurden(subjectId, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
    const { outputTax, inputTax, outputAmount } = await aggregateInvoices(subjectId, start, end);
    if (outputAmount <= 0)
        return null; // 无销售额无法计算
...
// 2. 进销项比对异常：进项税额环比增长
async function detectInputOutputMismatch(subjectId, period, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
...
async function detectRevenueCostMismatch(subjectId, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
...
// 4. 发票异常：作废率 / 顶额开票 / 连号
async function detectInvoiceAbnormal(subjectId, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
    ...
    const parts = [];
    if (voidRate > 0)
        parts.push(`作废率 ${(voidRate * 100).toFixed(1)}%`);
    if (topRate > 0)
        parts.push(`顶额开票占比 ${(topRate * 100).toFixed(1)}%`);
    if (hasConsecutive)
        parts.push(`连号发票 ${agg.consecutiveCount} 组`);
    return {
        hit: true,
        level,
...
async function detectInventoryMismatch(subjectId, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
...
// 6. 资金流异常：资金回流检测
async function detectFundFlowAbnormal(subjectId, start, end, thresholdHigh, thresholdMedium, _thresholdLow) {
    ...
    if (!roundTripCp || !wantRoundTrip)
        return null;
    // 高风险阈值配置了 roundTrip 则 HIGH，否则 MEDIUM
    const level = tHigh?.roundTrip === true ? 'HIGH' : 'MEDIUM';
    return {
        hit: true,
        level,
...
async function detectRelatedTransaction(subjectId, start, end, thresholdHigh, thresholdMedium, _thresholdLow) {
    // 关联方关键字
    ...
    let matchedAmount = 0;
    for (const inv of invoices) {
        const names = [inv.buyerName, inv.sellerName].filter(Boolean);
...
async function detectLongZeroFiling(subjectId, period, _start, _end, thresholdHigh, thresholdMedium, _thresholdLow) {
...
// 9. 期末留抵异常：留抵税额占进项比例
async function detectExcessCredit(subjectId, start, end, thresholdHigh, thresholdMedium, thresholdLow) {
...
async function notifyAdmins(eventId, subjectId, level, description) {
    try {
        await auditRepo_1.default.create({
...
async function scanSubject(subjectId, indicatorCodes, period) {
    // 校验主体存在
    ...
    const indicators = await prisma_1.default.riskIndicator.findMany({ where, orderBy: { id: 'asc' } });
    ...
    for (const ind of indicators) {
        ...
        let result = null;
        try {
            result = await detector(subjectId, resolvedPeriod, start, end, ind.thresholdHigh, ind.thresholdMedium, ind.thresholdLow);
        }
        catch (err) {
            // 单指标检测异常不影响其他指标
            // eslint-disable-next-line no-console
        ...
        hit++;
        // 去重：查找同主体+指标+期次的活跃事件
        const existing = await prisma_1.default.riskEvent.findFirst({
            where: {
                subjectId,
                indicatorId: ind.id,
        ...
        if (existing) {
            ...
            // 新建风险事件
            const created = await prisma_1.default.riskEvent.create({
                data: {
                    subjectId,
                    indicatorId: ind.id,
                    period: resolvedPeriod,
...
// subjectId 为空则扫描所有主体
async function rescanAll(subjectId) {
    const subjectWhere = {};
    if (subjectId)
        subjectWhere.id = subjectId;
    const subjects = await prisma_1.default.taxpayerSubject.findMany({ where: subjectWhere });
    const results = [];
    for (const subject of subjects) {
        ...
        let closed = 0;
        // 关闭不再命中的活跃事件
        const activeEvents = await prisma_1.default.riskEvent.findMany({
            where: { subjectId: subject.id, status: { in: ACTIVE_EVENT_STATUSES } },
        });
        // 本次扫描命中事件的 indicatorCode+period 集合
        const hitKeys = new Set(summary.events.map((e) => `${e.indicatorCode}|${summary.period}`));
        for (const ev of activeEvents) {
            const ind = await prisma_1.default.riskIndicator.findUnique({ where: { id: ev.indicatorId } });
...
function triggerScan(subjectId, source) {
    ...
    setImmediate(() => {
        scanSubject(subjectId)
            .then((summary) => {
            // eslint-disable-next-line no-console
            console.log(`[风险扫描] 主体 ${subjectId} 扫描完成（来源:${source}）：扫描 ${summary.scanned} 指标，命中 ${summary.hit}`);
        })
            .catch((err) => {
            // eslint-disable-next-line no-console
...
// ============ 钩子函数：供凭证服务/导入服务调用 ============
// 凭证过账后触发扫描
function onVoucherPosted(subjectId) {
    triggerScan(subjectId, 'VOUCHER_POSTED');
}
// 数据导入后触发扫描
function onDataImported(subjectId, dataType) {
