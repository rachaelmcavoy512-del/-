...
exports.parsePeriodRange = parsePeriodRange;
exports.getVATPayable = getVATPayable;
exports.generateVATReturn = generateVATReturn;
exports.generateCITReturn = generateCITReturn;
exports.generateSurTaxReturn = generateSurTaxReturn;
exports.generateIITReturn = generateIITReturn;
exports.generateStampTaxReturn = generateStampTaxReturn;
...
// 按科目编码获取该主体的科目记录
async function getAccountByCode(subjectId, code) {
    return prisma_1.default.account.findFirst({ where: { subjectId, code } });
}
// 聚合某科目在指定日期范围内已过账凭证的借/贷方发生额
async function aggregateRange(subjectId, accountId, start, end) {
    ...
    return {
        debit: Number((result._sum.debit ?? new client_1.Prisma.Decimal(0)).toString()),
        credit: Number((result._sum.credit ?? new client_1.Prisma.Decimal(0)).toString()),
    };
}
// 计算某科目截至 start 之前的期初余额（按余额方向）
async function getOpeningBalance(subjectId, accountId, before) {
    ...
    const debit = Number((result._sum.debit ?? new client_1.Prisma.Decimal(0)).toString());
    const credit = Number((result._sum.credit ?? new client_1.Prisma.Decimal(0)).toString());
    return account.balanceDirection === 'DEBIT' ? debit - credit : credit - debit;
}
// 获取某科目在期间内的"自然方向发生额"（贷方科目取贷方，借方科目取借方）
// 用于归集收入、成本等的发生额
async function getPeriodOccurrence(subjectId, code, range) {
...
// 按主体+期间获取发票列表
async function getInvoicesByPeriod(subjectId, range, direction) {
...
// 小规模：应纳税额 = 销售额 × 征收率
async function getVATPayable(subjectId, period) {
...
// 生成增值税申报表
async function generateVATReturn(subjectId, period) {
    const subject = await getSubject(subjectId);
    const range = parsePeriodRange(period);
...
// 一般纳税人增值税申报表
async function generateGeneralVATReturn(subject, range) {
    // ---------- 附列资料(一) 销售额及销项税额 ----------
    ...
    const certifiedInput = inputInvoices.reduce((s, inv) => s + Number(inv.taxAmount.toString()), 0);
...
// 小规模纳税人增值税申报表
async function generateSmallScaleVATReturn(subject, range) {
    // 销售额：主营业务收入凭证（不含税）+ 发票销售额
    const voucherRevenue = await getPeriodOccurrence(subject.id, ACCT_MAIN_REVENUE, range);
    const outputInvoices = await getInvoicesByPeriod(subject.id, range, 'OUTPUT');
    const invoiceSales = outputInvoices.reduce((s, inv) => s + Number(inv.amountExclTax.toString()), 0);
...
async function generateCITReturn(subjectId, period, isAnnual = false) {
    const subject = await getSubject(subjectId);
    const range = parsePeriodRange(period);
    // 营业收入 = 主营业务收入 + 其他业务收入
...
async function generateSurTaxReturn(subjectId, period, urbanRate = 0.07) {
    const subject = await getSubject(subjectId);
    const range = parsePeriodRange(period);
    // 本期应纳增值税额（附加税计税依据）
...
async function generateIITReturn(subjectId, period) {
    const subject = await getSubject(subjectId);
    const range = parsePeriodRange(period);
...
async function generateStampTaxReturn(subjectId, period) {
    const subject = await getSubject(subjectId);
    const range = parsePeriodRange(period);
    // 购销合同金额：进项 + 销项发票不含税金额合计
    const invoices = await getInvoicesByPeriod(subjectId, range);
    const contractAmount = invoices.reduce((s, inv) => s + Number(inv.amountExclTax.toString()), 0);
    // 印花税税率：购销合同 0.03%
    const rate = 0.0003;
...
function periodLabelToCode(range) {
    if (range.kind === 'MONTH') {
