...
// 上传文件（multipart）
async function uploadFile(path, token, fields, file) {
    ...
    const opts = { method: 'POST', path, host: '127.0.0.1', port: TEST_PORT, headers: hdrs };
    return new Promise((resolve, reject) => {
        const req = http_1.default.request(opts, (res) => {
            const chunks = [];
            res.on('data', (c) => chunks.push(c));
...
async function main() {
    ...
    const oldSubject = await prisma_1.default.taxpayerSubject.findUnique({ where: { taxNumber } });
    if (oldSubject) {
        await prisma_1.default.taxpayerSubject.delete({ where: { id: oldSubject.id } });
    ...
    try {
        ...
        const hasOutputTax = detailResp.data.accounts.some((a) => a.code === '22210101');
        const hasInputTax = detailResp.data.accounts.some((a) => a.code === '22210102');
        ...
        const acctMap = new Map(acctResp.data.map((a) => [a.code, a.id]));
        const acct = {
        ...
        scanResp.data.events.forEach((e) => console.log(`    - ${e.indicatorCode} | level=${e.level} | status=${e.status}`));
        ...
        // 取首个 PENDING 事件
        const pendingEvent = eventsResp.data.items.find((e) => e.status === 'PENDING');
        ...
        const actions = new Set(auditListResp.data.items.map((a) => a.action));
