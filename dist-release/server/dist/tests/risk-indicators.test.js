...
async function seedAccounts(subjectId) {
...
async function createPostedVoucher(subjectId, voucherNo, voucherDate, summary, entries, userId = 1) {
...
async function cleanupSubject(subjectId) {
    await prisma_1.default.riskRemediation.deleteMany({ where: { event: { subjectId } } });
    await prisma_1.default.riskEvent.deleteMany({ where: { subjectId } });
    await prisma_1.default.invoice.deleteMany({ where: { subjectId } });
    await prisma_1.default.bankTransaction.deleteMany({ where: { subjectId } });
    ...
    await prisma_1.default.account.deleteMany({ where: { subjectId } });
    await prisma_1.default.taxpayerSubject.delete({ where: { id: subjectId } });
}
// 按税号清理旧主体（若存在）
async function cleanupByTaxNumber(taxNumber) {
    const old = await prisma_1.default.taxpayerSubject.findUnique({ where: { taxNumber } });
    if (old) {
...
const results = [];
// 记录单个指标命中结果
function recordResult(code, name, hit, level, detail) {
...
async function main() {
    ...
    // 0. 校验指标库
    const indicatorCount = await prisma_1.default.riskIndicator.count();
    console.log(`[0] 风险指标库：${indicatorCount} 条内置指标`);
    if (indicatorCount < 9) {
        console.error('  ✗ 指标库不足9条，请先运行 npm run seed');
        process.exit(1);
    }
    const indicators = await prisma_1.default.riskIndicator.findMany({ orderBy: { id: 'asc' } });
    console.log('  指标列表：');
    ...
    // 创建主体A并播种科目
    const subjectA = await prisma_1.default.taxpayerSubject.create({
        data: {
            name: '风险指标测试A公司',
    ...
    await prisma_1.default.bankTransaction.create({
        data: {
            subjectId: sidA,
    ...
    const acct = (code) => acctMapA.get(code);
    ...
    const summaryA = await (0, riskScanService_1.scanSubject)(sidA, undefined, CUR);
    console.log(`  扫描 ${summaryA.scanned} 个指标，命中 ${summaryA.hit} 个风险`);
    summaryA.events.forEach((e) => console.log(`    - ${e.indicatorCode} | level=${e.level} | status=${e.status}`));
    // 查询事件并验证各指标命中
    const eventsA = await prisma_1.default.riskEvent.findMany({
    ...
    recordResult('VAT_TAX_BURDEN', '增值税税负率异常', !!e1, e1?.level, e1 ? `税负率=${e1.metricValue}（销项1430-进项52000→0 / 销售额11000）` : '期望税负率<0.01命中HIGH');
    // 2. INPUT_OUTPUT_MISMATCH：进项环比增长 = (52000-1300)/1300 ≈ 39倍 > 50% → HIGH
    const e2 = byCodeA.get('INPUT_OUTPUT_MISMATCH');
    recordResult('INPUT_OUTPUT_MISMATCH', '进销项比对异常', !!e2, e2?.level, e2 ? `进项环比增长=${e2.metricValue}（上期1300→本期52000）` : '期望环比增长>50%命中HIGH');
    // 3. REVENUE_COST_MISMATCH：成本率 = 950/1000 = 0.95 > 0.9 → HIGH
    const e3 = byCodeA.get('REVENUE_COST_MISMATCH');
    recordResult('REVENUE_COST_MISMATCH', '收入成本匹配异常', !!e3, e3?.level, e3 ? `成本率=${e3.metricValue}（成本950/收入1000=0.95）` : '期望成本率>0.9命中HIGH');
    // 4. INVOICE_ABNORMAL：作废率 = 2/8 = 25% > 10% → HIGH
    const e4 = byCodeA.get('INVOICE_ABNORMAL');
    recordResult('INVOICE_ABNORMAL', '发票异常', !!e4, e4?.level, e4 ? `作废率25%（2/8）metric=${e4.metricValue}` : '期望作废率>10%命中HIGH');
    // 5. INVENTORY_MISMATCH：差异率 = |389000-4050|/389000 ≈ 0.99 > 0.3 → HIGH
    const e5 = byCodeA.get('INVENTORY_MISMATCH');
    recordResult('INVENTORY_MISMATCH', '库存账实不符', !!e5, e5?.level, e5 ? `差异率=${e5.metricValue}（进销差389000 vs 账面变动4050）` : '期望差异率>0.3命中HIGH');
    // 6. FUND_FLOW_ABNORMAL：同对手方收入10万+支出9.9万，净额≈0 → HIGH
    const e6 = byCodeA.get('FUND_FLOW_ABNORMAL');
    recordResult('FUND_FLOW_ABNORMAL', '资金流异常', !!e6, e6?.level, e6 ? `资金回流metric=${e6.metricValue}` : '期望检测到资金回流命中HIGH');
    // 7. RELATED_TRANSACTION：卖家含"集团"关键字 → 命中（deviation=0.25>0.2 → MEDIUM）
    const e7 = byCodeA.get('RELATED_TRANSACTION');
    recordResult('RELATED_TRANSACTION', '关联交易定价异常', !!e7, e7?.level, e7 ? `关联方「某某集团供应链有限公司」metric=${e7.metricValue}` : '期望命中关联方关键字');
    // 9. EXCESS_CREDIT：留抵占比 = (52000-1430)/52000 ≈ 0.97 > 0.5 → HIGH
    const e9 = byCodeA.get('EXCESS_CREDIT');
    recordResult('EXCESS_CREDIT', '期末留抵异常', !!e9, e9?.level, e9 ? `留抵占比=${e9.metricValue}（进项52000-销项1430→50570/52000）` : '期望留抵占比>50%命中HIGH');
    ...
    const subjectB = await prisma_1.default.taxpayerSubject.create({
        data: {
            name: '风险指标测试B公司',
    ...
    const summaryB = await (0, riskScanService_1.scanSubject)(sidB, undefined, CUR);
    console.log(`  扫描 ${summaryB.scanned} 个指标，命中 ${summaryB.hit} 个风险`);
    summaryB.events.forEach((e) => console.log(`    - ${e.indicatorCode} | level=${e.level} | status=${e.status}`));
    const eventsB = await prisma_1.default.riskEvent.findMany({
    ...
    recordResult('LONG_ZERO_FILING', '长期零申报/微利申报', !!e8, e8?.level, e8 ? `连续零申报metric=${e8.metricValue}` : '期望连续≥3期零申报命中HIGH');
    // ===== 汇总 =====
    console.log('\n========== 9类风险指标命中测试汇总 ==========');
