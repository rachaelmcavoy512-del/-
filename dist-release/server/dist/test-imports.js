...
function buildPayrollXlsxWithError() {
    const data = [
...
async function main() {
    ...
    // 0. 准备测试主体（如不存在则创建）
    let subject = await prisma_1.default.taxpayerSubject.findUnique({ where: { taxNumber: 'TEST_IMPORT_001' } });
    if (!subject) {
    ...
    const admin = await prisma_1.default.user.findUnique({ where: { username: 'admin' } });
    if (!admin)
        throw new Error('未找到 admin 用户，请先运行 npm run seed');
    ...
    await prisma_1.default.importBatch.deleteMany({ where: { subjectId } });
    // ===== 测试1：发票 XML 解析 + 去重 =====
    console.log('\n--- 测试1：发票 XML 导入（首次）---');
    const xmlBuf = buildInvoiceXml();
    const r1 = await (0, importService_1.importInvoices)(subjectId, xmlBuf, 'test_invoices.xml', adminId);
    ...
    const r2 = await (0, importService_1.importInvoices)(subjectId, xmlBuf, 'test_invoices_dup.xml', adminId);
    ...
    const r3 = await (0, importService_1.importInvoices)(subjectId, xlsxBuf, 'test_invoices.xlsx', adminId);
    ...
    // 验证入库数据
    const invCount = await prisma_1.default.invoice.count({ where: { subjectId } });
    ...
    const r4 = await (0, importService_1.importPayrolls)(subjectId, payrollBuf, 'test_payroll.xlsx', adminId, '2024-03');
    ...
    const batch4 = await prisma_1.default.importBatch.findUnique({ where: { id: r4.batch.id } });
    ...
    const r5 = await (0, importService_1.importBankTransactions)(subjectId, bankBuf, 'test_bank.xlsx', adminId);
    ...
    // 验证流水方向
    const bankTxns = await prisma_1.default.bankTransaction.findMany({ where: { subjectId }, orderBy: { id: 'asc' } });
    ...
    // ===== 测试6：工资表 period 格式校验 =====
    console.log('\n--- 测试6：工资表 period 格式校验 ---');
    const r6 = await (0, importService_1.importPayrolls)(subjectId, payrollBuf, 'test_bad_period.xlsx', adminId, '2024-13');
    ...
    // ===== 汇总 =====
    console.log('\n=== 测试汇总 ===');
    const batchCount = await prisma_1.default.importBatch.count({ where: { subjectId } });
    console.log(`  共产生 ${batchCount} 个导入批次`);
    console.log(`  发票: ${await prisma_1.default.invoice.count({ where: { subjectId } })} 条`);
    console.log(`  流水: ${await prisma_1.default.bankTransaction.count({ where: { subjectId } })} 条`);
    console.log(`  工资: ${await prisma_1.default.payrollRecord.count({ where: { subjectId } })} 条`);
    console.log('\n=== Task5 导入功能测试全部通过 ✅ ===');
}
main()
    .catch((err) => {
